                                    ...
                                    std::vector<CashFlow>& GeneratedFlows) const
{
    ...
    for (unsigned long i = 0; i < TheOption1->CashFlows(SpotValues, GeneratedFlows1); i++)
        Values1[i] = GeneratedFlows1[i].Amount;
    for (unsigned long i = 0; i < TheOption2->CashFlows(SpotValues, GeneratedFlows2); i++)
        Values2[i] = GeneratedFlows2[i].Amount;

    for (int i = 0; i < NumberOfTimes; i++) {
        GeneratedFlows[i].TimeIndex = (unsigned long) i;
        GeneratedFlows[i].Amount = std::abs(Values2[i] - Values1[i]);
    }
